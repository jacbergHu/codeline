...
void* ProcessData(void* para) {
    ...
    foreach(QFileInfo fileInfo, param->list) {
        ...
        if(isDebugfileName) {
            qDebug() << result.fileName << "总行数" << result.totalLines;
            qDebug() << result.fileName << "代码总行数" << result.codeLines;
            qDebug() << result.fileName << "注释总行数" << result.commentLines;
            qDebug() << result.fileName << "空白总行数" << result.blankLines;
        }
...
void CreateThread() {
    ...
    qDebug() << "文件总数：" << allFileList.count();
    qDebug() << "文件总行数：" << allFileResult.totalLines;
    qDebug() << "代码总行数：" << allFileResult.codeLines;
    qDebug() << "注释总行数：" << allFileResult.commentLines;
    qDebug() << "空白总行数：" << allFileResult.blankLines;
    qDebug() << "统计文件类型：" << filterFileType;
    exit(0);
}
void ReadFiles(QString dirPath, QStringList filter) {
    ...
    QStringList strings;
    strings << "*";
    QFileInfoList list = dir.entryInfoList(strings, QDir::AllEntries | QDir::NoDotAndDotDot, QDir::DirsFirst); //list表示过滤后的文件列表
    if(list.isEmpty()) {
        if(isDebugfileName) {
